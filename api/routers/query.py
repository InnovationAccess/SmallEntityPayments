...
def _build_sql(query: BooleanQuery) -> tuple[str, List[bigquery.ScalarQueryParameter]]:
    ...
    if "patent_assignments" in tables:
        join_on = ""
        if "patent_file_wrapper" in tables:
            join_on = f"JOIN `{settings.assignments_table}` AS a ON a.patent_number = p.patent_number"
        else:
    ...
    if "maintenance_fee_events" in tables:
        if from_parts:
            # Join on patent_number with the first table.
            base_alias = "p" if "patent_file_wrapper" in tables else "a"
            from_parts.append(
                f"JOIN `{settings.maintenance_table}` AS m ON m.patent_number = {base_alias}.patent_number"
            )
        else:
            from_parts.append(f"`{settings.maintenance_table}` AS m")
            select_parts.append("m.patent_number")
        select_parts.extend([
    ...
    sql = f"SELECT DISTINCT {', '.join(select_parts)} FROM {from_sql} {where} LIMIT {query.limit}"
    return sql, params
